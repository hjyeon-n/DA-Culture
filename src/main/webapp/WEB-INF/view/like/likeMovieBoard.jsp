...
<head>
	...
	<script> 
   	$(document).on('click', '#btnSearch', function(e){
		e.preventDefault();
		var url = "${pageContext.request.contextPath}/userLikesMovie";
    ...
    <div class="container">
        <div class="table-wrapper">
            <table class="table table-striped">
                ...
                <tbody>
                    <c:forEach var="userLikesMovieReviewList" items="${userLikesMovieReviewList}">
						<tr>
							<td>${userLikesMovieReviewList.movieReviewID}</td>
							<td>
								<c:choose>
								<c:when test="${fn:length(userLikesMovieReviewList.movieTitle) > 10}">
	    							${fn:substring(userLikesMovieReviewList.movieTitle, 0, 10)}...
	 							</c:when>
	 							<c:otherwise>
	 								${userLikesMovieReviewList.movieTitle}
	 							</c:otherwise>
	 						</c:choose>
							</td>
							<td><a href="${pageContext.request.contextPath}/movieBoardDetail?movieReviewID=${userLikesMovieReviewList.movieReviewID}">
							<c:choose>
								<c:when test="${fn:length(userLikesMovieReviewList.movieReviewTitle) > 18}">
	    							${fn:substring(userLikesMovieReviewList.movieReviewTitle, 0, 18)}...
	 							</c:when>
	 							<c:otherwise>
	 								${userLikesMovieReviewList.movieReviewTitle}
	 							</c:otherwise>
	 						</c:choose>
	 						</a></td>
							<td>
							<c:choose>
								<c:when test="${fn:length(userLikesMovieReviewList.movieReviewPreview) > 30}">
	    							${fn:substring(userLikesMovieReviewList.movieReviewPreview, 0, 30)}...
	 							</c:when>
	 							<c:otherwise>
	 								${userLikesMovieReviewList.movieReviewPreview}
	 							</c:otherwise>
	 						</c:choose>
							</td>
							<td>${userLikesMovieReviewList.movieScore}</td>
							<td>${userLikesMovieReviewList.userID}</td>
						</tr>				
					</c:forEach> 
                </tbody>
            </table>
