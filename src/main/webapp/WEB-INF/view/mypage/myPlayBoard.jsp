...
<head>	
	...
	<script> 
   	$(document).on('click', '#btnSearch', function(e){
		e.preventDefault();
		var url = "${pageContext.request.contextPath}/myPlayBoard";
    ...
    <div class="container">
        <div class="table-wrapper">
            <table class="table table-striped">
                ...
                <tbody>
                    <c:forEach var="myPlayReviewList" items="${myPlayReviewList}">
						<tr>
							<td>${myPlayReviewList.playReviewID}</td>
							<td>
							<c:choose>
								<c:when test="${fn:length(myPlayReviewList.playTitle) > 10}">
	    							${fn:substring(myPlayReviewList.playTitle, 0, 10)}...
	 							</c:when>
	 							<c:otherwise>
	 								${myPlayReviewList.playTitle}
	 							</c:otherwise>
	 						</c:choose>
	 						</td>
							<td><a href="${pageContext.request.contextPath}/playBoardDetail?playReviewID=${myPlayReviewList.playReviewID}">
							<c:choose>
								<c:when test="${fn:length(myPlayReviewList.playReviewTitle) > 18}">
	    							${fn:substring(myPlayReviewList.playReviewTitle, 0, 18)}...
	 							</c:when>
	 							<c:otherwise>
	 								${myPlayReviewList.playReviewTitle}
	 							</c:otherwise>
	 						</c:choose>
	 						</a></td>
							<td>
							<c:choose>
								<c:when test="${fn:length(myPlayReviewList.playReviewPreview) > 30}">
	    							${fn:substring(myPlayReviewList.playReviewPreview, 0, 30)}...
	 							</c:when>
	 							<c:otherwise>
	 								${myPlayReviewList.playReviewPreview}
	 							</c:otherwise>
	 						</c:choose>
							</td>
							<td>${myPlayReviewList.playScore}</td>
							<td>${myPlayReviewList.userID}</td>
						</tr>				
					</c:forEach> 
                </tbody>
            </table>
