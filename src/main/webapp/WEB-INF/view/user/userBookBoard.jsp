...
<head>
	...
	<script> 
   	$(document).on('click', '#btnSearch', function(e){
		...
		var url = "${pageContext.request.contextPath}/userBookBoard";
    ...
    <div class="container">
        <div class="table-wrapper">
            <table class="table table-striped">
                ...
                <tbody>
                    <c:forEach var="userBookReviewList" items="${userBookReviewList}">
						<tr>
							<td>${userBookReviewList.bookReviewID}</td>
							<td>
							<c:choose>
								<c:when test="${fn:length(userBookReviewList.bookTitle) > 10}">
	    							${fn:substring(userBookReviewList.bookTitle, 0, 10)}...
	 							</c:when>
	 							<c:otherwise>
	 								${userBookReviewList.bookTitle}
	 							</c:otherwise>
	 						</c:choose>
	 						</td>
							<td><a href="${pageContext.request.contextPath}/bookBoardDetail?bookReviewID=${userBookReviewList.bookReviewID}">
							<c:choose>
								<c:when test="${fn:length(userBookReviewList.bookReviewTitle) > 18}">
	    							${fn:substring(userBookReviewList.bookReviewTitle, 0, 18)}...
	 							</c:when>
	 							<c:otherwise>
	 								${userBookReviewList.bookReviewTitle}
	 							</c:otherwise>
	 						</c:choose>
	 						</a></td>
							<td>
							<c:choose>
								<c:when test="${fn:length(userBookReviewList.bookReviewPreview) > 30}">
	    							${fn:substring(userBookReviewList.bookReviewPreview, 0, 30)}...
	 							</c:when>
	 							<c:otherwise>
	 								${userBookReviewList.bookReviewPreview}
	 							</c:otherwise>
	 						</c:choose>
							</td>
							<td>${userBookReviewList.bookScore}</td>
							<td>${userBookReviewList.userID}</td>
						</tr>				
					</c:forEach> 
                </tbody>
            </table>
